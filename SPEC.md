...
- 「じゅんびOK!(確定)」ボタンは設定個数すべてを配置し終えるまで無効。
...
- 「⚠ 画面を消さないでください(宝の位置が失われます)」の警告を常時表示。
- 見つける側が「探索スタート」ボタンを押すと探索ターン開始。ボタンのタップがWebXRのselectとしても発火し、開始直後に意図しない発掘(穴掘り)になる誤爆を防ぐため、探索開始から約0.7秒間はタップを発掘試行と見なさない(インターバルも発生しない)。
- ARセッションは裏で維持し続ける。

...
- **置き直し可**: 再タップするたびにプレビューが移動。「ここに埋める(確定)」ボタンで確定。
- 時間切れ時は最後に置いた位置で自動確定。
...
- カメラ映像上をタップ = **その場所をスコップで掘る**。タップ光線の hit-test で3D点を取得し、宝箱アンカーとの**3D距離**で判定。宝箱は地中に埋まっており、**カメラに隠し場所が映っただけでは発見にならない**。掘ったときに初めて判定される。
  - 距離 ≤ **成功半径(デフォルト50cm、設定変更可)** → 発掘成功。**地中から宝箱がポンッと飛び出す**演出(跳ね上がり+蓋オープン+パーティクル)+ファンファーレ。
  - 失敗 → スコップで掘る演出(土煙エフェクト+効果音)。**掘った跡(穴+土盛り)がその場所に残り続け**、既に掘った場所が見た目で分かる。
- **スコップのインターバル**: 掘った後 **デフォルト3秒**(設定変更可)は再度掘れない。インターバル中は画面にリング状の残り時間を表示し、タップを受け付けない(レーダーは動作し続ける)。

...
- **あそびかた(ゲームモード)を設定画面で選択**(デフォルト: 宝箱モード):
  ...
  - **コインモード**: 埋めずに常時可視化されたコインを隠し、見つける側がタップして回収する(3.2b/3.4b)。見つけやすいようコインはわずかに発光する。
...
- **確定後もコインは非表示にしない**(常時可視化の仕様のため、手渡し・探索ターンを通してAR空間上に見えたまま)。
...
### 3.4b 見つけるターン(コインモード)
- コインは埋まっておらず**常時可視化**されているため、カメラに映れば見た目で見つけられる。レーダー(3.5)は宝箱モードと同様に、隠したコインとの3D距離に応じたおおまかな方向・距離の手がかりを与える。
...
- 時間切れ時、未回収のコインは元々可視化されているため、宝箱モードのような「ポンッ」という開示演出・効果音は発生しない。
